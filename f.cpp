...
void GetSiftedUp(std::vector<std::pair<long long, long long>>& heap,
                 std::vector<long long>& index_vec, long long the_index) {
  if (the_index != 1) {
    ...
    if (heap[the_index].first < heap[tmp_ind].first) {
      ...
      GetSiftedUp(heap, index_vec, tmp_ind);
    }
  }
}

void GetSiftedDown(std::vector<std::pair<long long, long long>>& heap,
                   std::vector<long long>& index_vec, long long the_index) {
  if (2 * the_index < (long long)heap.size()) {
    ...
    if (heap[the_index].first > heap[tmp_ind].first) {
      std::swap(index_vec[heap[the_index].second],
                index_vec[heap[tmp_ind].second]);
      std::swap(heap[the_index], heap[tmp_ind]);
      GetSiftedDown(heap, index_vec, tmp_ind);
    }
  }
}

void GetInserted(std::vector<std::pair<long long, long long>>& heap,
                 std::vector<long long>& index_vec, long long value,
                 long long position) {
  heap.push_back(std::make_pair(value, position));
  long long the_index = (long long)heap.size() - 1;
  index_vec[position] = the_index;
  GetSiftedUp(heap, index_vec, the_index);
}

void GetDecreased(std::vector<std::pair<long long, long long>>& heap,
  ...
  GetSiftedUp(heap, index_vec, index_vec[index]);
}
...
int main() {
  ...
  std::cin >> quantity;
  std::vector<std::pair<long long, long long>> heap(0);
  heap.push_back(std::make_pair(-kInf, -1));
  long long value = 0;
  std::string input;
  ...
  for (long long tmp = 0; tmp < quantity; ++tmp) {
    ...
    if (input == "insert") {
      std::cin >> value;
      GetInserted(heap, index_vec, value, tmp + 1);
    } else if (input == "decreaseKey") {
      GetDecreased(heap, index_vec);
    } else if (input == "extractMin") {
      GetExtracted(heap, index_vec);
